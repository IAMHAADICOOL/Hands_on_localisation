...
class MapFeature:
    ...
    * **The storage representation:** which is the representation used to store the feature in the map within the state vector.
    ...
    def hfj(self, xk_bar, Fj):  # Observation function for zf_i and x_Fj
        ...
        # robot_pose = Pose3D(self.robot.xsk[0:3,0].reshape(3,1))
        _hfj = self.s2o(NxF.boxplus(xk_bar.ominus()))
        # _hfj = self.s2o(NxF.boxplus(robot_pose.ominus()))
        return _hfj
    ...
    def g(self, xk, BxFj):  # xBp [+] (BxFj + vk)
        ...
        # TODO: To be implemented by the student
        # NxFj = self.Feature(self.o2s(BxFj)).boxplus(xk)


        NxFj = xk.boxplus(self.Feature(self.o2s(BxFj)))
        return NxFj
    ...
    def Jgx(self, xk, BxFj):  # Jacobian wrt xk of the inverse sensor model for a single feature observation
        ...
        # TODO: To be implemented by the student
        # Jp = self.Feature(self.o2s(BxFj)).J_1boxplus(xk)
        # only returning Jp, not Jnp for this lab
        Jnp = 0

        Jp = xk.J_1boxplus(self.Feature(self.o2s(BxFj)))
        return Jp
    ...
    def Jgv(self, xk, BxFj):
        ...
        """
        # TODO: To be implemented by the student
        # J = self.Feature(self.o2s(BxFj)).J_2boxplus(xk)
        J = xk.J_2boxplus(self.Feature(self.o2s(BxFj)))@self.J_o2s(BxFj)
        # J = xk.J_2boxplus((self.GetFeatures(self.o2s(BxFj))))@self.J_o2s(BxFj)
        # only returning Jp, not Jnp for this lab
        return J
...
class Cartesian2DMapFeature(MapFeature):
    ...
    def GetFeatures(self, xk_bar):
        ...
        """
        # TODO: To be implemented by the student
        zf, Rf = self.robot.ReadCartesianFeature(self.robot.xsk)
