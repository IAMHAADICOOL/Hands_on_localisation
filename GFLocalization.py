...
class GFLocalization(Localization,GaussianFilter):
    ...
    #     # 1. Initialize log arrays on the first run
    #     if not hasattr(self, 'log_x'):
    #         self.log_x = np.zeros((xk.shape[0], self.kSteps))
    #         self.log_sigma = np.zeros((xk.shape[0], self.kSteps))
    #         self.log_x_bar = np.zeros((xk_bar.shape[0], self.kSteps))
    #         self.log_xs = np.zeros((xsk.shape[0], self.kSteps))

    #     # 2. Dynamic Resizing: Check if xk has grown (New Features Added)
    #     current_dim = self.log_x.shape[0]
    #     new_dim = xk.shape[0]
        
    #     if new_dim > current_dim:
    #         rows_to_add = new_dim - current_dim
    #         zeros_block = np.zeros((rows_to_add, self.kSteps))
            
    #         # Resize log_x and log_sigma
    #         self.log_x = np.vstack((self.log_x, zeros_block))
    #         self.log_sigma = np.vstack((self.log_sigma, zeros_block))
            
    #     # 3. Dynamic Resizing: Check if xk_bar has grown
    #     # xk_bar grows in the prediction step of the *next* iteration after features are added
    #     current_bar_dim = self.log_x_bar.shape[0]
    #     new_bar_dim = xk_bar.shape[0]

    #     if new_bar_dim > current_bar_dim:
    #         rows_to_add = new_bar_dim - current_bar_dim
    #         zeros_block = np.zeros((rows_to_add, self.kSteps))
            
    #         # Resize log_x_bar
    #         self.log_x_bar = np.vstack((self.log_x_bar, zeros_block))

    #     # 4. Log the data
    #     # Now that arrays are resized, these assignments will work safely
    #     self.log_xs[0:xsk.shape[0], self.k] = xsk.T
    #     self.log_x[0:xk.shape[0], self.k] = xk.T
    #     self.log_sigma[0:xk.shape[0], self.k] = np.sqrt(np.diag(Pk)).T
    #     self.log_x_bar[0:xk_bar.shape[0], self.k] = xk_bar.T
